...
import {
  FiFileText, FiPlusCircle, FiBarChart2, FiLogOut,
  FiMenu, FiX, FiSun, FiMoon, FiArrowLeft,
} from 'react-icons/fi';
import { useAuth } from '@/providers/AuthProvider';
import { useTheme } from '@/hooks/useTheme';

const sidebarLinks = [
  { href: '/creator-dashboard', label: 'Analytics', icon: FiBarChart2 },
  { href: '/creator-dashboard/add-prompt', label: 'Add Prompt', icon: FiPlusCircle },
  { href: '/creator-dashboard/my-prompts', label: 'My Prompts', icon: FiFileText },
];
    ...
    <div className="min-h-screen bg-base-200/30">
      <div className="lg:hidden fixed top-0 left-0 right-0 z-50 glass border-b border-base-300/50 h-16 flex items-center justify-between px-4">
        <Link href="/" className="flex items-center gap-2.5">
          ...
          <span className="font-bold text-sm tracking-tight">Creator</span>
        </Link>
          ...
          >
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto p-4">
                <CreatorSidebarContent pathname={pathname} logout={logout} />
              </div>
            </motion.aside>
          </motion.div>
        )}
        ...
        <div className="p-5 border-b border-base-300">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <span className="font-bold text-lg tracking-tight block leading-none">Creator</span>
...
function CreatorSidebarContent({ pathname, logout }: { pathname: string; logout: () => void }) {
  return (
    <nav className="space-y-1">
      <p className="text-[10px] font-semibold text-base-content/30 uppercase tracking-wider px-3 mb-2">Creator Tools</p>
