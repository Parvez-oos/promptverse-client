...
import { z } from 'zod';
import { FiUser, FiMail, FiLock, FiEye, FiEyeOff, FiArrowRight, FiImage } from 'react-icons/fi';
...
export default function RegisterPage() {
  ...
  const onSubmit = async (data: RegisterForm) => {
    setLoading(true);
    try {
      await registerUser({ name: data.name, email: data.email, password: data.password, photoURL: data.photoURL });
      router.push('/');
    } catch (error: any) {
      ...
      >
        <div className="card bg-base-100 border border-base-300 p-8 shadow-xl">
          <div className="text-center mb-8">
            ...
            <p className="text-base-content/55 text-sm mt-1">Join PromptVerse today</p>
          </div>
          ...
          <div className="relative mb-6">
            ...
            <div className="relative flex justify-center text-xs">
              <span className="bg-base-100 px-4 text-base-content/40">or register with email</span>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div>
              ...
              <div className="relative">
                <FiUser className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-base-content/40" />
                <input
                  type="text"
                  {...register('name')}
            ...
            <div>
              <label className="text-sm font-medium mb-1.5 block">Photo URL (optional)</label>
              <div className="relative">
                <FiImage className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-base-content/40" />
                <input
                  type="url"
                  {...register('photoURL')}
