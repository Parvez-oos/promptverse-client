...
export default function MyPromptsPage() {
  ...
  const fetchFn = user?.role === 'creator' || user?.role === 'admin'
    ? () => dashboardService.getMyPrompts({
        ...(search && { search }),
        page: page.toString(),
        limit: '15',
      })
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ['myPrompts', user?._id, page, search],
    queryFn: fetchFn,
    enabled: !!user,
  });
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-base-content/50">
            {pagination ? `${pagination.total} prompts` : 'Manage your created prompts'}
          </p>
        </div>
        <Link href="/dashboard/add-prompt" className="btn btn-primary btn-sm rounded-lg gap-1.5">
      ...
      {prompts.length === 0 ? (
        <EmptyState
          ...
          description="Create your first prompt to share with the community."
          action={<Link href="/dashboard/add-prompt" className="btn btn-primary btn-sm rounded-lg">Create Prompt</Link>}
        />
      ) : (
            ...
            <tbody>
              {prompts.map((prompt: any) => (
                <tr key={prompt._id} className="hover:bg-base-200/50">
                  ...
                  <td>
                    <span className={`badge badge-sm ${prompt.visibility === 'private' ? 'badge-accent' : 'badge-ghost'}`}>
                      {prompt.visibility}
                    </span>
                  </td>
                  ...
                  <td className="text-right">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <Link href={`/dashboard/my-prompts?edit=${prompt._id}`} className="btn btn-ghost btn-xs btn-square" title="Edit">
